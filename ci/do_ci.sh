...
if [[ "$1" == "cmake.test" ]]; then
  ...
  /usr/bin/bazel $BAZEL_STARTUP_OPTIONS build $BAZEL_OPTIONS_ASYNC --//api:with_abseil=true //...
  /usr/bin/bazel $BAZEL_STARTUP_OPTIONS test $BAZEL_TEST_OPTIONS_ASYNC --//api:with_abseil=true //...
  exit 0
